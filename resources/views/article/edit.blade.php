    ...
    <div class="min-h-screen flex items-center justify-center">
        <div class="w-full max-w-lg border p-8 rounded-2xl shadow-xl bg-white">
            ...
            @if ($errors->any())
                ...
                @endforeach
            @endif
            <form action="{{ route('editArticleAction') }}" method="POST" id="articleForm" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    ...
                    <input type="text"
                        ...
                        placeholder="Enter Title Article" name="title" value="{{ $data->title }}" />
                </div>
                <div class="mb-4">
                ...
                <button class="w-full p-3 rounded-lg bg-blue-500 hover:bg-blue-600 text-white font-semibold hover:opacity-90">
                    Edit Article
                </button>
            </form>
            <form action="{{ route('deleteArticle') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus artikel ini?')">
                @csrf
                @method('DELETE')
                <input type="hidden" name="id" value="{{ $data->id }}">
                <button class="w-full p-3 rounded-lg bg-red-500 hover:bg-red-600 text-white font-semibold hover:opacity-90 mt-2">
                    Delete Article
                </button>
            </form>
        </div>
